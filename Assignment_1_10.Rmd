...
# 1.1: building model1
model1 <- lm(loan_amount ~ payment_to_income_ratio, data = df_sample)
summary(model1)

# 1.2: making predictor model
library(modelr)

model1_pred <- df_sample %>%
  add_predictions(model1) %>% 
  rename(predicted_loan_amount = pred)
#view(model1_pred)
View(model1_pred) #-> i think you need to capitalize otherwise it won't work
...
model2 <- lm(loan_amount ~ payment_to_income_ratio + annual_income, data = df_sample)
summary(model2)
```
...
model3 <- lm(loan_amount ~ payment_to_income_ratio + annual_income + loan_to_income_ratio, data = df_sample)
summary(model3)
```
...
model4 <- lm(loan_amount ~ payment_to_income_ratio + annual_income + product_type + years_employed + age + loan_status + defaults_on_file + loan_intent + credit_score, data = df_sample)
summary(model4)

model5 <- lm(loan_amount ~ payment_to_income_ratio + annual_income + product_type + age + loan_status, data = df_sample)
summary(model5)
...
mean(residuals(model5))
```
### Assumption 4: Homoscedasticity sjors
```{r}
#(1) refers to a plot of residuals 
